  ...
  <main id="main" class="main">
                ...
                if(isset($_GET['editid'])){
                    ...
                    <form class="row g-3" action="core/update.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12">
                            ...
                            <select id="inputState" class="form-select" name="parent_cat">
                            <option>Choose...</option>
                                <?php 
                                $category_sql = "SELECT * FROM mart_category WHERE parent_cat='0' ORDER BY c_name ASC";
                                $category_res = mysqli_query($db,$category_sql);
                                while($row = mysqli_fetch_assoc($category_res)){
                                    $cat_id     = $row['ID'];
                                    $cat_name   = $row['c_name'];
                                    $cat_image  = $row['c_image'];
                                    $cat_parent = $row['parent_cat'];
                    ...
                    <form class="row g-3" action="core/insert.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12">
                            ...
                            <option selected="">Choose...</option>
                                <?php 
                                $category_sql = "SELECT * FROM category WHERE parent_cat='1' ORDER BY c_name ASC";
                                $category_res = mysqli_query($db,$category_sql);
                                while($row = mysqli_fetch_assoc($category_res)){
                                    $cat_id     = $row['ID'];
                                    $cat_name   = $row['c_name'];
                                    ...
                                    $cat_parent = $row['parent_cat'];
                                    $cat_status = $row['c_status'];
                                 ?><option value="<?php echo $cat_id;?>"><?php echo $cat_parent;?></option><?php
                                }
    ...
    <?php 
        ...
        if(isset($_GET['deleteid'])){
            ...
            delete_file('c_img','category','ID',$delid,'assets/img/products/category/');

            DEL('category','ID', $delid,'category.php');
